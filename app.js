...
app.get("/api/players/:id", requireAuth, async (req, res) => {
  try {
    const p = await Player.collection.findOne(playerIdQuery(req.params.id));
...
app.put("/api/players/:id", requireAuth, async (req, res) => {
  try {
    ...
    const query = playerIdQuery(rid);
    const update = await Player.collection.updateOne(query, { $set: req.body });
    if (update.matchedCount === 0) return res.status(404).json({ error: "غير موجود" });
    const p = await Player.collection.findOne(query);
    res.json(p);
  } catch (err) { res.status(500).json({ error: err.message }); }
});

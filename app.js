...
app.get("/api/players", requireAuth, async (req, res) => {
  try {
    ...
    const fixed = await Promise.all(players.map(async p => {
      if (!p.id) {
        const newId = p._id.toString();
        await Player.updateOne({ _id: p._id }, { $set: { id: newId } });
        p.id = newId;
      }
      return p;
    }));
...
app.put("/api/players/:id", requireAuth, async (req, res) => {
  try {
    let p = await Player.findOneAndUpdate({ id: req.params.id }, req.body, { new: true });
    if (!p && mongoose.Types.ObjectId.isValid(req.params.id)) {
      p = await Player.findByIdAndUpdate(req.params.id, req.body, { new: true });
    }
...
app.delete("/api/players/:id", requireAuth, async (req, res) => {
  try {
    let deleted = await Player.findOneAndDelete({ id: req.params.id });

...
async function seedAdmin() {
  try {
    ...
    if (count === 0) {
      // Only runs once — when the database has no admins at all
      const passwordHash = await hashPassword("hadaf2024");
      await Admin.create({ id: "1", username: "admin", passwordHash, isSuperAdmin: true });
      console.log("Bootstrap admin created: admin / hadaf2024 — change password immediately!");
    } else {
      console.log(`Admin accounts loaded from database (${count} account(s))`);
    }

  ...
  try {
    ...
    const fixed = await Promise.all(players.map(async p => {
      ...
      if (String(p.id) !== idStr) {
        await Player.updateOne({ _id: p._id }, { $set: { id: idStr } });
        p.id = idStr;
      }
      return p;
    }));
...
app.put("/api/players/:id", requireAuth, async (req, res) => {
  try {
    ...
    let p = await Player.findOneAndUpdate({ id: rid }, req.body, { new: true });
    if (!p) p = await Player.findOneAndUpdate({ id: Number(rid) }, { ...req.body, id: rid }, { new: true });
    if (!p && mongoose.Types.ObjectId.isValid(rid)) p = await Player.findByIdAndUpdate(rid, req.body, { new: true });
...
app.delete("/api/players/:id", requireAuth, async (req, res) => {
  try {
    const rid = req.params.id;
    let deleted = await Player.findOneAndDelete({ id: rid });
    if (!deleted) deleted = await Player.findOneAndDelete({ id: Number(rid) });
    if (!deleted && mongoose.Types.ObjectId.isValid(rid)) await Player.findByIdAndDelete(rid);
    res.json({ ok: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
